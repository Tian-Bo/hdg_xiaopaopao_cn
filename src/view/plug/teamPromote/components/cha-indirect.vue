 ...
 * @Author: tianbo
 * @LastEditors: tianbo
 * @Date: 2019-10-09 18:12:26
 * @LastEditTime: 2019-10-09 18:12:26
 -->
<template>
    <div>
        <!-- main begin-->
                ...
                <tbody v-for="(item, index) in dataList.list" :key="index">
                    <tr class="reg-list-tbody">
                        ...
                        <td>
                            ...
                            <span style="color: #c6c6c6;">（{{ item.one_money.desc }}）</span>
                        </td>
                        <td>
                            ¥{{ item.two_money.two_money }}
                            <span style="color: #c6c6c6;">（{{ item.two_money.desc }}）</span>
                        </td>
                        <td>
                            <router-link
                                tag="a"
                                :to="'/channeRelation?active_id=' + active_id + '&user_id=' + item.id + '&model_id=' + model_id + '&channel_id=' + channel_id + '&tab=' + 2"
                                style="color: #409eff"
                            >详情</router-link>
                        </td>
...
<script>
// 获取api
import { getChannelRecommend } from "@/api/api";
export default {
    data() {
        return {
            ...
            page: 1, // 分页
            dataList: [], // 数据列表
            model_id: this.$route.query.model_id || "", // 模型id
            active_id: this.$route.query.active_id || "", // 活动id
            channel_id: this.$route.query.channel_id || "" // 渠道id
        };
    },
    created() {
        if (this.tab == 2) {
    ...
    methods: {
        ...
        getDatalist() {
            ...
            this.$http.get(getChannelRecommend, { params }).then(res => {
                this.loading = false;
