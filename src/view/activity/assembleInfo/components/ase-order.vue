    ...
    <div class="ase-value">
        ...
        <div class="header">
            ...
            <div class="header-item header-item-border">
                <span>进度</span>
                <p><span class="header-item-f20">{{ finish }}</span>/{{ diff }}</p>
            </div>
            <div class="header-item ">
                <span>开团时间/到期时间</span>
                <p style="font-size: 14px;">{{ begin_time }}</p>
                <p style="font-size: 14px; margin-top: 0;">{{ end_time }}</p>
            </div>
        </div>
        <!-- header-end-->
        ...
        <transition name="fade" mode="out-in">
            <div class="ase-value-details" v-show="aseOrderBtnState">
                <!--第一行 商户下拉选择器  报名时间 -->
        ...
        <!-- 表格 -->
        <div>
            <aseList :management="management" :loading="loading" />
        </div>
        <!-- 分页 -->
        <el-pagination
...
<script>
import aseList from "./ase-list";
// 获取api
import { getManageFitter, getManagement } from "@/api/api";
export default {
    props: ['aseOrderBtnState'],
    ...
    created() {
        ...
        this.begin_time = this.$route.query.begin_time;
        this.end_time = this.$route.query.end_time;
        this.finish = this.$route.query.finish;
        this.diff = this.$route.query.diff;
